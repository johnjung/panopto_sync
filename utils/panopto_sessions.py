...
class PanoptoSessions:
    ...
    def get_access(self, session_id):
        '''
        Call GET /api/v1/sessions/{id}/settings/access API and return the response
        '''
        while True:
            url = 'https://{0}/Panopto/api/v1/sessions/{1}/settings/access'.format(self.server, session_id)
    ...
    def update_access(self, session_id, inherited, level):
        '''
        Call PUT /api/v1/sessions/{id}/settings/access API to update the name
        ...
        if self.get_access(session_id)['Level'] != level:
            try:
                while True:
                    url = 'https://{0}/Panopto/api/v1/sessions/{1}/settings/access'.format(self.server, session_id)
                    payload = {
                ...
                print('Update access failed. {0}'.format(e))
                return False
    ...
    def get_permissions(self, session_id):
        ...
        '''
        while True:
            url = 'https://{0}/Panopto/api/v1/sessions/{1}/permissions'.format(self.server, session_id)
            resp = self.requests_session.get(url = url)
            if self.__inspect_response_is_retry_needed(resp):
                continue
            data = resp.json()
