...
def main():
    ...
    print('1 - disable warnings')

    oauth2 = PanoptoOAuth2(args.server, args.client_id, args.client_secret, not skip_verify)

    print('2 - set up PanoptoOauth2 object')

    # Get data about DMA folders from Panopto.
    folder_mgr = PanoptoFolders(args.server, not skip_verify, oauth2)

    print('3 - get PanoptoFolders object')

    panopto_folders_list = folder_mgr.get_children(args.folder_id)

    print('4 - get_children')
    ...
    print('5 - report folder names that appear more than once')
    ...
    uploader = PanoptoUploader(args.server, not skip_verify, oauth2)
